<ul class="menu-inner py-1">
    ...
    config('constants.permissions.User management.Update.name'),
        ...
        <x-nav-link :href="route('users.index')" :active="request()->routeIs(['users.index', 'users.edit'])" icon="ti-users">
            {{ __('User') }}
        </x-nav-link>
    @endcanany
    @canany([
    ...
    @endcanany
    @can(config('constants.permissions.Audit Logs.List audit logs.name'))
        <x-nav-link :href="route('audits.index')" :active="request()->routeIs('audits.index')" icon="ti-article">
            {{ __('Audit Logs') }}
        </x-nav-link>
    @endcan
    @canany([

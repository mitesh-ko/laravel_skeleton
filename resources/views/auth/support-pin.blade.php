            ...
            <div class="authentication-inner py-4">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <h6>Enter support pin to continue.</h6>
                        @if(session('message'))
                            <div class="alert alert-warning">
                                <h6 class="alert-heading mb-1">{{ session('message') }}</h6>
                            </div>
                        @endif
                        <form method="POST" action="{{ route('supportPinVerify.check') }}">
                            @csrf
                        ...
                        @if(session('backToAccount', false))
                            <form action="{{ route('loginAs', session('backToAccount')) }}" method="POST">
                                @csrf
                                <button class="btn btn-success d-grid w-100 mb-3 mt-3">My Account</button>
                            </form>
                        @endif
                    </div>
                </div>
